from core.formating import format_name, format_subtext, format_faction, format_number, format_card_text, format_text, \
    format_illustrator, format_set, color_picker, faction_order, format_victory
...
def format_player_card(c):
    ...
    text = "%(faction)s %(name)s%(subtext)s%(level)s\n" \
           "%(type)s %(slot)s\n" \
           "%(traits)s" \
           "%(costs)s" \
           "%(icons)s\n" \
...
def format_inv_card_f(c):
    formater = {"class": format_faction(c),
                ...
                "pack": format_set(c),
                "traits": "<b>%s</b>\n" % c['traits'],
                "taboo_text": "%s" % format_taboo_text(c['code']),
...
def format_player_card_short(c, qty=0):
    formater = {"name": "%s" % c['name'],
                ...
                "subname": ": <i>%s</i>" % c['subname'] if ("subname" in c and "Campaign Log" in c['real_text']) else ""
                }
